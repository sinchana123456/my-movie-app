...
const Cart = (props) => {
    ...
    const newEmailId = localStorage.getItem('email')
        ...
        const fetch = async () => {
            const res = await axios.get(`https://crudcrud.com/api/b45af7ceb16d43dea7a2736d1dc9c152/cart${newEmailId}`);
    ...
    <Modal onClose={props.onClose}>
      <CartCloseButton className={classes.close} onClose={props.onClose} />
        ...
        <div className={classes.wrapper}>
          <span className={classes.item}>Movie</span>
          <span className={classes.price}>Price</span>
          <span className={classes.quantity}>Tickets</span>
          </div>
        <div>
            {cartItemsList}
